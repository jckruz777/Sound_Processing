...
def a3 (x, fm, plim=None, dt=None, n=None, tw=None, dlog2p=None, s0=None):
    ...
    for i in range(0, len(pc)): # para cada candidato:
        ...
        for j in range(0, X.shape[1]): # por cada ventana:
            A[:,j] = factor * interp(fi, f, X[:,j]) # estime el espectro en las armonicas
        si = sum(A, 0) # calcule el puntaje del candidato en cada ventana sumando el valor del espectro en las armonicas
